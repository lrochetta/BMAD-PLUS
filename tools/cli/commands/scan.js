...
function hasBmadInstalled(dir) {
  return fs.existsSync(path.join(dir, '.agents')) || fs.existsSync(path.join(dir, '_bmad'));
}
...
module.exports = {
  ...
  options: [
    ...
    [
      '--active-days <n>',
      'Days since last modified to consider a project "active" (default: 30)',
      '30',
    ],
    [
      '--paused-days <n>',
      'Days since last modified to consider a project "paused" (default: 180)',
      '180',
    ],
  ...
  action: async (options) => {
    ...
    const maxDepth =
      !isNaN(rawDepth) && rawDepth > 0
        ? rawDepth
        : (() => {
            clack.log.warn(`Invalid --depth "${options.depth}", defaulting to 4`);
            return 4;
          })();

    const rawActive = parseInt(options.activeDays, 10);
    const activeDays =
      !isNaN(rawActive) && rawActive > 0
        ? rawActive
        : (() => {
            clack.log.warn(`Invalid --active-days "${options.activeDays}", defaulting to 30`);
            return 30;
          })();

    const rawPaused = parseInt(options.pausedDays, 10);
    const pausedDays =
      !isNaN(rawPaused) && rawPaused > 0
        ? rawPaused
        : (() => {
            clack.log.warn(`Invalid --paused-days "${options.pausedDays}", defaulting to 180`);
            return 180;
          })();
    ...
    clack.log.info(
      `    ${pc.green('●')} active    modified < ${activeDays} days ago     ${pc.dim(`(${activeCount} found)`)}`
    );
    clack.log.info(
      `    ${pc.yellow('◐')} paused    modified ${activeDays}–${pausedDays} days ago   ${pc.dim(`(${pausedCount} found)`)}`
    );
    clack.log.info(
      `    ${pc.dim('○')} archived  modified > ${pausedDays} days ago    ${pc.dim(`(${archivedCount} found)`)}`
    );
    clack.log.info('');
