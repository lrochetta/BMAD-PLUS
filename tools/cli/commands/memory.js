  ...
  action: async (subcommand, options) => {
    ...
    if (cmd === 'status') {
      ...
      if (hasProjectMemory) {
        ...
        clack.log.info(
          `  decisions.md     ${decisions.exists ? pc.green('✓') : pc.red('✗')}  ${decisions.count} entries  ${pc.dim(decisions.lastModified || '')}`
        );
        clack.log.info(
          `  lessons.md       ${lessons.exists ? pc.green('✓') : pc.red('✗')}  ${lessons.count} entries  ${pc.dim(lessons.lastModified || '')}`
        );
        clack.log.info(
          `  patterns.md      ${patterns.exists ? pc.green('✓') : pc.red('✗')}  ${patterns.count} entries  ${pc.dim(patterns.lastModified || '')}`
        );
        clack.log.info(
          `  context.md       ${context.exists ? pc.green('✓') : pc.red('✗')}  ${pc.dim(context.lastModified || '')}`
        );
        clack.log.info(
          `  sessions/        ${sessions > 0 ? pc.green('✓') : pc.dim('·')}  ${sessions} handoff(s)`
        );
      ...
      if (hasGlobalBrain) {
        ...
        clack.log.info(
          `  decisions.md     ${gDecisions.exists ? pc.green('✓') : pc.dim('·')}  ${gDecisions.count} entries`
        );
        clack.log.info(
          `  lessons.md       ${gLessons.exists ? pc.green('✓') : pc.dim('·')}  ${gLessons.count} entries`
        );
        clack.log.info(
          `  patterns.md      ${gPatterns.exists ? pc.green('✓') : pc.dim('·')}  ${gPatterns.count} entries`
        );
      ...
      const exportPath = path.join(
        projectDir,
        `bmad-brain-export-${new Date().toISOString().slice(0, 10)}.md`
      );
      const sections = [];
        ...
        for (const f of fs.readdirSync(projectsDir)) {
          if (f.endsWith('.yaml')) {
            sections.push(
              `### ${f}`,
              '```yaml',
              fs.readFileSync(path.join(projectsDir, f), 'utf8').trim(),
              '```',
              ''
            );
          }
