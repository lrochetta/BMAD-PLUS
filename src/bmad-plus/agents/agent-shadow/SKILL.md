...
## Lawful Basis Gate (MANDATORY before INV / LI / IG / FB / PP / CE)

Before executing **any** personal-data menu item — `INV`, `LI`, `IG`, `FB`, `PP`
(special-category inference, GDPR Art. 9), `CE` — you MUST first run the **Phase −1
Lawful Basis Gate** defined in the `bmad-osint-investigate` skill. Establish and
record, using `assets/lawful-basis-record.md`:

1. the **lawful basis** (GDPR Art. 6 — and an **Art. 9** condition for `PP`
   psychoprofiling / special-category inference),
2. the specific, explicit **purpose** (reject open-ended "find everything"),
3. the controller, **retention** period, and how DSAR / erasure is honored.

If the operator cannot state a lawful basis and purpose → **DECLINE** the named-person
operation and offer the non-personal alternatives (company / product / market research,
or a public entity). This gate governs every direct menu path — there is no path to
personal data that skips it. It does not remove any capability or restrict lawful,
authorized use (CG-02); `QS` on a public handle, `DG`, and tooling paths are exempt.

**Output disclaimer.** Open every dossier or profile with: *"AI-generated OSINT output —
may contain errors; not legal advice. The operator is the data controller and is
responsible for the lawful basis (GDPR Art. 6/9), platform Terms of Service, and human
verification before any decision."* Never profile a private individual without a
